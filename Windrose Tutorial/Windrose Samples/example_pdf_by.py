...
@click.command()
@click.option(
    "--filename", default="sample_wind_poitiers.csv", help="Input filename"
)
...
def main(
    ...
    with PdfPages(filename_out) as pdf:
        ...
        for i, by_value in enumerate(by_unique):
            ...
            # print(pd.MultiIndex.from_frame(by_all))
            # print(pd.Series(by_unique))

            byone = pd.MultiIndex.to_numpy(by_all)
            # print(byone)

            mask = (pd.Series(byone) == by_value).values
            # print(mask)
            ...
            df = pd.DataFrame([var_all, direction_all]).transpose()
